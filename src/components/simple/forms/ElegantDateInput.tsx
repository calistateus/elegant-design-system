...
export function ElegantDateInput({
  ...
  function segmentStyle(isFocused: boolean, isError: boolean): React.CSSProperties {
    return {
      ...
      border: `1px solid ${isError ? 'var(--color-error-border)' : disabled ? 'var(--color-interactive-disabled-border)' : isFocused ? 'var(--color-border-input-focus)' : 'var(--color-border-input)'}`,
      borderRadius: 'var(--size-input-radius)',
      outline: 'none',
      boxShadow: !disabled && isFocused && !isError ? 'var(--shadow-focus-ring)' : 'none',
