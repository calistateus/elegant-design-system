...
export function ElegantPinInput({
  ...
  const getCellStyle = (index: number): React.CSSProperties => ({
    ...
    border: `1px solid ${disabled ? 'var(--color-interactive-disabled-border)' : focusedIndex === index ? 'var(--color-border-input-focus)' : 'var(--color-border-input)'}`,
    borderRadius: 'var(--size-input-radius)',
    outline: 'none',
    boxShadow: !disabled && focusedIndex === index ? 'var(--shadow-focus-ring)' : 'none',
